...
function sync_verticalScrollbarMove(distance) {
  const { data, table, selector } = this;
  data.sync_scrolly(distance, () => {
    selector.resetBRLAreaOffset();
    editorSetOffset.call(this);
    table.render();
  });
...
function sync_horizontalScrollbarMove(distance) {
  const { data, table, selector } = this;
  data.sync_scrollx(distance, () => {
    selector.resetBRTAreaOffset();
    editorSetOffset.call(this);
    table.render();
  });
...
function toolbarChange(type, value) {
  ...
  } else if (type === 'overlayer') {
     this.enableOverlayer(value);
  } else {
...
//    this.tableEl = h('canvas', `${cssPrefix}-table`);
    this.tableEl = h('div', `${cssPrefix}-table`);
    // resizer
  ...
  sync_movex(x) {
    sync_horizontalScrollbarMove.call(this,x);
  }
